    ...
    <>
      ...
      <nav className="hidden lg:flex fixed top-10 left-28 right-8 xl:left-32 xl:right-12 z-50 pointer-events-auto transition-all duration-300">
        <div className="w-full bg-white/95 backdrop-blur-xl shadow-[0_4px_24px_rgba(0,0,0,0.06)] border border-white/60 rounded-full flex items-center justify-between px-6 xl:px-8 py-3">
          ...
          {/* Middle: Links */}
          <div className="flex items-center gap-6 xl:gap-10 text-[14px] xl:text-[16px] font-normal text-neutral-900 tracking-wide" style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 400 }}>
          ...
          <div className="shrink-0 pl-2">
            <Link to="/contact" className="px-6 py-2.5 rounded-full border border-rose-400/60 text-neutral-900 font-normal text-[15px] tracking-wide hover:bg-rose-50 hover:text-rose-600 transition-all flex items-center justify-center" style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 400 }}>
              Enroll Now
            </Link>
          </div>
      ...
      <div className={`lg:hidden fixed inset-0 z-40 transition-all duration-500 ${
        ...
        <div className={`absolute right-0 top-0 bottom-0 w-[85%] max-w-sm bg-white/98 backdrop-blur-xl border-l border-white/40 shadow-luxury transition-transform duration-500 ${
          ...
          <div className="h-full overflow-y-auto p-6 pt-24 space-y-2" style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 400 }}>
            <a href="https://maps.google.com/?q=Rizeworld+Institute+of+AI+and+Digital+Marketing,+Alwar" target="_blank" rel="noopener noreferrer" className="block py-3 px-4 rounded-xl font-normal text-neutral-900 hover:bg-neutral-200 hover:text-rize-blue transition-colors">
              Location Alwar, RJ
            </a>
            ...
            <Link
              to="/contact"
              onClick={() => setMobileOpen(false)}
              className="block mt-6 text-center py-3 rounded-full border border-rose-300 text-neutral-900 font-normal hover:bg-rose-50 transition-colors"
...
function MobileLink({ to, label, onClick }: any) {
  return (
    <Link
      to={to}
      onClick={onClick}
      className="block py-3 px-4 rounded-xl font-normal text-neutral-900 hover:bg-neutral-200 hover:text-rize-blue transition-colors"
    >
      {label}
    </Link>
  );
